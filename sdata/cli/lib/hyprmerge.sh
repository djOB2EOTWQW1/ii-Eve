...
REPO_CONFIG="${1:-}"
LOCAL_CONFIG="${2:-$HOME/.local/share/ii-eve/hyprland.conf}"
VERBOSE="$IS_VERBOSE"
    ...
    if echo "$trimmed" | grep -q "="; then
        ...
        if [[ "$field" == "animation" ]]; then
            ...
            if grep -q "animation = $anim_name" "$LOCAL_CONFIG"; then
                ...
                eve hyprset anim "$anim_name" "$full_params" >/dev/null 2>&1 || true
            fi
            continue
        fi
        ...
        if [[ -n "$current_section" ]]; then
            if key_exists_in_section "$current_section" "$field"; then
                 ...
                 eve hyprset key "${current_section}:${field}" "$value" >/dev/null 2>&1 || true
                 sleep 0.05
            fi
        else
            ...
            if [[ "$field" =~ ^(layerrule|windowrule|windowrulev2|bind|exec|env|monitor)$ ]]; then
                ...
                if grep -qE "^[[:space:]]*${field}[[:space:]]*=" "$LOCAL_CONFIG"; then
                    ...
                    apply "$field"
                    eve hyprset key "$field" "$value" >/dev/null 2>&1 || true
                fi
            fi
        fi
        continue
